...
internal class Program
{
	private static int Main(string[] args)
	{
		...
		static void ExportPackageCache(PackageWithWinMd pkg, string cacheDir, JsonSerializerOptions jsonOptions)
		{
			...
			File.WriteAllText(Path.Combine(cacheDir, "meta.json"), JsonSerializer.Serialize(value, WinMdJsonContext.Default.PackageMeta));
			File.WriteAllText(Path.Combine(cacheDir, "namespaces.json"), JsonSerializer.Serialize(list2, WinMdJsonContext.Default.ListString));
			foreach (string item in list2)
			{
				...
				File.WriteAllText(Path.Combine(text, path), JsonSerializer.Serialize(value2, WinMdJsonContext.Default.ListWinMdTypeInfo));
			}
		}
		...
		static void PrintUsage()
		{
			...
			Console.Error.WriteLine("  members <TypeName>              List members of a type");
		...
		static int RunUpdate(CliArgs cli)
		{
			...
			foreach (string item2 in list)
			{
				...
				else
				{
					...
					File.WriteAllText(Path.Combine(text3, text4 + ".json"), JsonSerializer.Serialize(value, WinMdJsonContext.Default.ProjectManifest));
				}
			}
