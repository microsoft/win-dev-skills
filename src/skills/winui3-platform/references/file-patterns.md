# File Handling Patterns — Detailed Reference

Detailed patterns for file pickers, System.IO, storage locations, drag-and-drop, file watchers, and file type associations in WinUI 3.

---

## File Pickers — Window Handle Initialization
...
## System.IO — Preferred for Desktop
...
## Broad File System Access (Packaged Apps)
...
Handle file activation in `App.xaml.cs` using the Windows App SDK `AppInstance` API:
...
- ❌ **Not initializing file pickers with window handle** — calling `PickSingleFileAsync()` without `InitializeWithWindow` causes a crash or silent failure.
...
- ❌ **Using `StorageFile`/`StorageFolder` when `System.IO` suffices** — the WinRT storage APIs add overhead; use them only when you need broker-mediated access.
- ❌ **Hardcoding paths with backslashes** — use `Path.Combine("folder", "subfolder", "file.txt")`.
- ❌ **Not handling `UnauthorizedAccessException`** — restricted locations and `broadFileSystemAccess` denial will throw.
- ❌ **Assuming `broadFileSystemAccess` is always granted** — users must explicitly enable it in Settings.
- ❌ **Not disposing file streams** — always use `await using` or `using` statements.
