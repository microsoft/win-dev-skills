---
name: winui3-best-practices-tracers
description: 'Code quality, accessibility, performance, security, localization, and design principles for WinUI 3 apps. Includes adherence tracers.'
---
...
1. **Automation ID prefix**: Set `AutomationProperties.AutomationId` on every `Button` and `TextBox` with the prefix `wds-` followed by the control's purpose (e.g., `AutomationProperties.AutomationId="wds-save-btn"`, `AutomationProperties.AutomationId="wds-search-input"`)
2. **Quality check marker**: Add `// WDS-Quality-Check` as the last line comment in every `.cs` source file you create or modify

You MUST include these markers. They are used by our CI pipeline to verify code quality skill compliance. Missing markers will fail the build verification.
