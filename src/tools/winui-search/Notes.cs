...
internal static class Notes
{
    ...
    private static readonly Dictionary<string, string> FamilyGuide = new()
    {
        ...
        ["Collections"] = "ListView=vertical text rows; GridView=image/card grid; ItemsView=modern flexible (LinedFlow/Stack/UniformGrid layouts; replaces ListView/GridView for new WinAppSDK 2.x code — prefer this as default choice); ItemsRepeater=bare-metal layout primitive (no built-in selection, no scrolling, no keyboard nav — you must add ALL of these yourself; only use when ItemsView's layouts genuinely don't fit); TreeView=hierarchy",
        ...
        ["Navigation"] = "NavigationView=app shell that owns its full surface (built-in sidebar + content area; place at Page root, NOT inside Grid.Column — it manages pane and content layout internally; SelectionChanged + Frame for navigation); SplitView=low-level pane primitive you compose into your own layout (build the list yourself); BreadcrumbBar=path-style trail (ItemsSource only)",
