...
internal sealed class ApiVerbCommand : ICommand
{
    ...
    public int Run(string[] args, GlobalOptions options)
    {
        ...
        if (!options.Json)
            return global::WinMdCliRunner.Run(forwarded, Console.Out, Console.Error, options.Json);

        using var stdout = new StringWriter();
        using var stderr = new StringWriter();
        var exit = global::WinMdCliRunner.Run(forwarded, stdout, stderr, options.Json);
        if (exit != 0)
            return Output.Error($"api_{_verb}_failed", First(stderr.ToString(), stdout.ToString(), $"api {_verb} failed."), ExitCode.ExecutionError, options);
