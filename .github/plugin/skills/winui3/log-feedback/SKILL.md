---
name: log-feedback
description: Logs issues and friction to a session temp file (.feedback-session.md) during development.
---
...
At the start of any workflow, if `.feedback-session.md` exists and is non-empty, a previous session was interrupted. Follow the [reflect-session skill](../reflect-session/SKILL.md) to merge and clear it before starting new work.
...
**When to log** — stop and append to `.feedback-session.md` immediately when any of the following occur:
