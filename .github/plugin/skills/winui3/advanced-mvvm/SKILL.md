...
Use source generators for all ViewModels. Combine `[RelayCommand]` with `CanExecute`, `[NotifyCanExecuteChangedFor]`, `[NotifyPropertyChangedFor]`, and partial `OnPropertyChanged` hooks.
...
Use `WeakReferenceMessenger` for decoupled ViewModel-to-ViewModel communication. Implement `IRecipient<T>` on `ObservableRecipient` subclasses. Call `OnActivated()`/`OnDeactivated()` to manage subscriptions.
...
Model explicit states with an enum. Bind `VisualStateManager` to the state property — avoid scattered boolean flags like `IsLoading`, `HasError`.
...
### 6. Validation with ObservableValidator
...
Use `[RelayCommand(IncludeCancelCommand = true)]` for cancellation. Track progress with an `[ObservableProperty]`. Compose commands sequentially for dependent operations.
...
### 8. Dialog and Navigation Services
...
1. **No code-behind commands** — Event-to-command bindings use XAML Behaviors or `x:Bind`, never `+=` handlers.
2. **CanExecute guards present** — State-dependent commands use `CanExecute` + `[NotifyCanExecuteChangedFor]`.
3. **Messenger cleanup** — Messenger-using ViewModels extend `ObservableRecipient` and call `OnDeactivated()`.
4. **Async commands use CancellationToken** — Long-running commands accept `CancellationToken` with `IncludeCancelCommand = true`.
5. **State enum for page states** — Single enum property replaces scattered booleans (`IsLoading`, `HasError`).
6. **Services behind interfaces** — `ContentDialog`, `Frame.Navigate`, pickers never called directly from ViewModels.

## Must Read & Research

> **Agent rule:** Before generating advanced MVVM code, look up the latest CommunityToolkit.Mvvm documentation using the microsoft-docs skill. API surfaces change across versions.

| Topic | Reference |
|---|---|
| CommunityToolkit.Mvvm source generators | https://learn.microsoft.com/dotnet/communitytoolkit/mvvm/ |
| ObservableValidator and validation | https://learn.microsoft.com/dotnet/communitytoolkit/mvvm/observablevalidator |
| WeakReferenceMessenger | https://learn.microsoft.com/dotnet/communitytoolkit/mvvm/messenger |
| XAML Behaviors for WinUI 3 | https://github.com/Microsoft/XamlBehaviors |
| AsyncRelayCommand patterns | https://learn.microsoft.com/dotnet/communitytoolkit/mvvm/asyncrelaycommand |
| WinUI 3 navigation patterns | https://learn.microsoft.com/windows/apps/design/basics/navigate-between-two-pages |
