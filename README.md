...
| `Microsoft.WindowsAppSDK.Analyzers.dll` | `winui-dev-workflow` skill (prebuilt) — source under `src/tools/winui3-analyzer/Microsoft.WindowsAppSDK.Analyzers/` | A Roslyn analyzer that flags common WinUI 3 / Windows App SDK pitfalls at build time. Committed alongside the skill so the skill is self-contained, and verified against source on every PR by the `analyzer-provenance` CI job. | Publish as `Microsoft.WindowsAppSDK.Analyzers` on NuGet (the csproj is already wired for it). Skill stops shipping the prebuilt DLL — projects pick it up via `<PackageReference>`. |
