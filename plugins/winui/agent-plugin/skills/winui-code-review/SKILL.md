...
Read through the project's XAML and C# files and check each section below. Before reporting an API mismatch or recommending a replacement member, confirm it against the restored project's own references with `winapp find-api` (WinApp CLI 0.7+) — for example `winapp find-api members NavigationView --filter selected --json` — so the review is grounded in the packages the app actually ships, not in a remembered signature. See [winui-design](../winui-design/SKILL.md) for batch lookups and project scoping.
