...
Before the first final action, persist one run-identity record containing the launch command, returned PID, selected HWND, target project/output fingerprint, and launch time. Include that run ID, PID, and HWND in every final action, tree, screenshot, and health record. If the process exits or a foreground diagnostic restart is required, end that run; never use evidence from the replacement process to complete the prior run's sequence.
