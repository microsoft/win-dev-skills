...
If debug output or WinUI triage terminates without an app-owned cause, permit one focused temporary instrumentation pass before forming another structural hypothesis. Before changing media, composition, WebView, template, navigation, or another suspected subsystem, record entry, exit, and failure at the narrowest app-owned constructors, handlers, and transition helpers already on the action path. Observe `UnhandledException` without marking it handled. Use the result to establish which app-owned boundary completed and one stable exception or transition timeline; do not instrument unrelated features, replace the subsystem first, or treat logging as a fix. Remove all temporary instrumentation before the final analyzer build and clean replay. If this pass still yields no actionable signature, retain the truthful failed state and continue only through the existing bounded probe rules.
...
After progressive coverage and all target changes, close the development process and start one fresh no-build process from the canonical initial state. Replay the primary sentinel first, then every runnable planned state in order, capturing new evidence. Only this final falsification pass may set target states to `passed` and resolve their runtime findings. Close every app-owned target window used by the run afterward and confirm that the returned primary PID and those HWNDs disappeared.

Before the first final action, persist one run-identity record containing a stable run ID, the launch command, returned primary PID, initial HWND, target project/output fingerprint, and launch time. Include that run ID and the artifact's actual HWND in every final action, tree, screenshot, and health record. Associate a window only when it is the primary window, an app-owned secondary window, or a dialog or picker whose owner chain resolves to the launched app. Record the observed owner PID when the command exposes it; otherwise retain the window inventory that established the association. A secondary or associated window can open or close without ending the run. If the primary process exits, a foreground diagnostic restart is required, or the target fingerprint changes, end that run; never use evidence from the replacement run to complete the prior run's sequence.
...
Before writing those summaries, reconcile all three durable records as one closure transaction: final evidence updates the state plan, the same replay closes or reopens linked findings, and the resulting state IDs/statuses update `validation`. Evidence for an attempted state while its plan entry remains `not-run`, or evidence satisfying a finding while that finding remains open, invalidates the completion claim until the records agree.
