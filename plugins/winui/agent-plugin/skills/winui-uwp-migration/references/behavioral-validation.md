...
If debug output or WinUI triage terminates without an app-owned cause, permit one focused temporary instrumentation pass before forming another structural hypothesis. Observe `UnhandledException` without marking it handled, and record entry, exit, and failure only at app-owned boundaries already present on the active action path. Use the result to establish a stable exception or transition timeline; do not instrument unrelated features or treat logging as a fix. Remove all temporary instrumentation before the final analyzer build and clean replay. If this pass still yields no actionable signature, retain the truthful failed state and continue only through the existing bounded probe rules.
