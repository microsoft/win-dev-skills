...
2. Read it once.
3. Confirm `schemaVersion` is `1.3`, `status` is `mechanical-migration-complete`, `mechanicalVerification.status` is `passed`, `mechanicalVerification.targetProjectGraph.status` is `passed` or `not-required`, no `UWMIG013` is pending, and `source.projectFile` identifies the exact entry project selected above. A missing or different entry project invalidates the report; rerun migration from the correct containing directory instead of continuing with partial evidence. A `failed` or `incomplete` target project graph must be corrected in the existing target and refreshed with `winapp migrate verify` before semantic edits or a build.
...
Use `activationAnalysis` as the deterministic inventory of protocol and file-association declarations, not as evidence that activation behavior works. For every activation contract in scope, follow [Activation contracts](references/activation-contracts.md) to implement one target-owned routing boundary and prove its runtime outcome. A translated declaration never resolves `UWMIG002` without a successful replay through the corresponding Windows activation mechanism.

Use `mechanicalVerification.targetProjectGraph` as structured evidence that the entry target project does not consume files owned by a nested target project through SDK defaults or explicit items. For each issue, choose the target architecture: move the nested project outside the entry project's item scope, or add active exclusions and item removals in a contained project, props, or targets file that participates in the build. Then run `winapp migrate verify "<target>"` and require `passed` or `not-required` before the first build. The CLI detects and verifies ownership; this skill chooses the architecture. A direct plus transitive `ProjectReference` path is not itself a collision, so do not invent duplicate-reference restrictions or manually edit `targetProjectGraph` or `UWMIG013`.
...
Keep the migrated project portable and target-owned. It must not consume files from the original UWP checkout, a fixture directory, or a machine-specific absolute path. For linked source `Content` or `PRIResource`, copy the required bytes under the target root—normally at the source `Link` path—and reference that target-relative file. When a source MSBuild expression resolves deterministically to one file, use `copied-linked-content`; the CLI verifies the resolved source and target hashes. If the expression cannot be resolved or the CLI cannot record the correct contained architecture, leave `UWMIG012` pending and report the limitation. Never preserve or introduce an external link merely to satisfy the verifier; `migrate verify` rejects active target file items that resolve outside the target root or use rooted machine-specific values.
...
When the active sentinel traverses a custom template for a framework-owned control, or a transition dynamically changes navigation-item topology together with selection or lifetime, treat that path as a migration-sensitive seam even when namespace conversion and compilation succeed. Decide the target representation before porting the source template or collection mutation, then preserve the observable source outcome instead of assuming that the source visual tree or control-owned collection remains valid. Follow [Platform semantic differences](references/platform-semantic-differences.md) for the bounded review.
...
After the current migration slice, run `winapp migrate verify "<target>"` before the first build when the patch changed project/build files, resource files or dictionaries, copied/deleted files, activation declarations, recorded project-item evidence, namespaces in bulk, or anything relevant to `targetProjectGraph`. Do not build while that graph is `failed` or `incomplete` or while `UWMIG013` is pending. Do not run verify after ordinary C# API fixes, before every later build, or after runtime experiments. Do not repeat its successful checks with `rg`.
...
Before this update, record every implemented, deterministically verifiable review-required project-item decision with `winapp migrate decide-project-item`, then run `winapp migrate verify "<target>"` if a CLI-owned mechanical-risk file or decision evidence changed since its last passing result. Confirm `mechanicalVerification.status` is `passed`, `targetProjectGraph.status` is `passed` or `not-required`, no `UWMIG013` is pending, and every mechanically eligible `activationAnalysis` contract has `verificationStatus: verified`. Do not manually edit `activationAnalysis`, `projectItemDecisions`, `mechanicalVerification`, `UWMIG011`, `UWMIG012`, or `UWMIG013`; the CLI owns them. This final check does not replace build or runtime evidence.
