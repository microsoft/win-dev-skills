...
Complete reference for mapping WPF types to WinUI 3 equivalents when migrating .NET WPF applications.
...
| `DataGrid` | `CommunityToolkit.WinUI.UI.Controls.DataGrid` | Requires CommunityToolkit NuGet |
...
- `UseWPF` → `UseWinUI`
- TFM: `net8.0-windows` → `net8.0-windows10.0.19041.0` (minimum Windows 10 version required)
- Add `WindowsPackageType=None` for unpackaged desktop apps
- Add `SelfContained=true` + `WindowsAppSDKSelfContained=true` for self-contained deployment
- Add `DISABLE_XAML_GENERATED_MAIN` to `DefineConstants` if using a custom `Program.cs` entry point
- Move icon from `Resources/` to `Assets/`

### XAML ApplicationDefinition Setup

WinUI 3 requires explicit `ApplicationDefinition` declaration if `App.xaml` is not at the project root:

```xml
<ItemGroup>
  <Page Remove="SubFolder\App.xaml" />
</ItemGroup>
<ItemGroup>
  <ApplicationDefinition Include="SubFolder\App.xaml" />
</ItemGroup>
```
...
If your project references native C++/WinRT components:

```xml
<PropertyGroup>
  <CsWinRTIncludes>MyNativeComponent</CsWinRTIncludes>
  <CsWinRTGeneratedFilesDir>$(OutDir)</CsWinRTGeneratedFilesDir>
</PropertyGroup>
```

This generates C# projection classes for WinRT components, allowing you to call native APIs from managed code.
